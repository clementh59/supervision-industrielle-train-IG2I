...
void Envoi_Tableau_Octet(int sockfd, int addrGuest, int addrDest, int addr_premier_objet, BOOL mot, int nb_obj, char *tableau) {
    ...
    buffer[13] = 0x06; // Code catégorie (0-7)
    ...
    for (int i = 0; i < nb_obj; i++) {
        if (mot) {
            buffer[20 + i * 2] = tableau[i] & 0x00FF;
            buffer[21 + i * 2] = tableau[i] & 0xFF00;
            len += 2;
        } else {
    ...
    printf("La longueur du buffer est %d\n", buffer[5]);
    /*printf("\n\n");
    write(sockfd, buffer, len);
    sleep(1);
    read(sockfd, rbuffer, sizeof(rbuffer));
...
int main() {
    ...
    // A1b //// 0x01f6 = %MW502
    //tableau1[0] = 31;Envoi_Tableau_Octet(sockTCP, 0x801B, 0x8017, 0x01f6, TRUE, 0x0001, tableau);

    // Tn0 //// 0x01f4 = %MW500
    tableau1[0] = 16;Envoi_Tableau_Octet(sockTCP, 0x801b, 0x8017, 0x01f4, TRUE, 0x0001, tableau1);

    tableau1[0] = 100;Envoi_Tableau_Octet(sockTCP, 0x801b, 0x8017, 0x01f6, TRUE, 0x0001, tableau1);
